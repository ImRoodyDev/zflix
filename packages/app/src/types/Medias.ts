...
export type ProviderModuleManifest = {
	/** Unique scheme identifier for the provider (e.g., `"bunny"`, `"open-subtitles"`) */
	scheme: string;
...
export type SourcesResult<T extends MediaSource | MediaSubtitleSource> = {
	sources: T[] | null;
	providers: ProviderModuleManifest[];
};
...
export interface MediaSource extends SourceProvider<'m3u8' | 'dash' | 'mp4' | 'webm' | 'mkv' | 'flv' | 'avi' | 'mov'> {
	playlist:
		| {
				bandwidth: number;
...
export interface MediaSubtitleSource extends SourceProvider<'srt' | 'vtt'> {
	languageName: string;
	url: string;
}
