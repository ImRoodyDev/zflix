...
function EpisodeButton({ episode, onPress, runtime }: Props) {
	...
	const image = useMemo(() => {
		return (
			<Image
				source={episode.backdrop ? getApiUrl(episode.backdrop) : undefined}
	...
	return (
		<InView onChange={(inView) => setIsVisible(inView)} triggerOnce={true}>
			<CustomButton
				//..
				className={clsx('app-episode-item', isHovered && 'app-episode-item-hover')}
				onPress={handlePress}
				enableRipple
				...
				{() => (
					<>
						<View className={'app-episode-img-ptn'}>
							{isVisible && image}
							<View className={'app-episode-img-overlay'} style={isHovered && { backgroundColor: '#0000005f' }}>
								{isHovered && (
									<Animated.View entering={ZoomIn} exiting={ZoomOut} className={'app-episode-round-play'}>
									...
									runtime && runtime > 0 ? (
										<View className={'app-episode-dt-slider'}>
											<View
												...
												style={{ width: `${(runtime / episode.minutes) * 100}%` }}
											/>
										</View>
									) : (
										<></>
						...
						<View className={'app-episode-infos'}>
							<View className={'app-episode-hd'}>
								<View className={'app-episode-round-badge'}>
									<Text className={'app-episode-badge-number'}>{episode.number}</Text>
								</View>
							...
							<View className={'app-episode-badges'}>
								<View className={'app-episode-badge'}>
									<Text className={'preview-badge-txt episode-badge-txt'}>{episode.aired ?? '##-##-####'}</Text>
								</View>
								<View className={'app-episode-badge'}>
							...
							<Text className={'app-episode-summary'} numberOfLines={2} lineBreakMode={'tail'}>
								{episode.summary}
							</Text>
						</View>
					</>
