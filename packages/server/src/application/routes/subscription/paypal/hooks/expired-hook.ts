...
import { sendSubscriptionExpired } from '@app/controllers/subscription';
...
router.post('/', async (req: Request, res: Response) => {
	try {
		...
		const sent = await sendSubscriptionExpired(subscription, 'en');
		if (!sent) {
			Logger.error(`[WEBHOOK] Subscription ${subscription.id} expired but expiration email failed to send`);
		}

		const success = new HttpSuccess({ message: 'Subscription expired notification sent successfully' });
