...
router.post('/', async (req: Request, res: Response) => {
	try {
		...
		if (!sent) {
			Logger.error(`[WEBHOOK] Subscription ${subscription.id} reactivated but activation email failed to send`);
		}

		const success = new HttpSuccess({ message: 'Reactivated sent successfully' });
