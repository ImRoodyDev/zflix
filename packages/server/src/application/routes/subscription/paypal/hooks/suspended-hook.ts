...
import { sendSubscriptionSuspended } from '@app/controllers/subscription';
...
router.post('/', async (req: Request, res: Response) => {
	try {
		...
		const sent = await sendSubscriptionSuspended(subscription, 'en');
		if (!sent) {
			Logger.error(`[WEBHOOK] Subscription ${subscription.id} suspended but suspension email failed to send`);
		}

		const success = new HttpSuccess({ message: 'Subscription suspended' });
