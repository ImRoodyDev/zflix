...
router.get('/', async (req: AuthenticatedRequest, res) => {
	try {
		...
		const sort = (req.query.srtc as string) || undefined;
		const sortGenre = (req.query.srtg as string) || undefined;
		...
		// Retrieve discover information
		const discover = await TMDBService[type == 'movies' ? 'discoverMovies' : 'discoverTv'](profile.certificationId, page, lang, {
			sort,
			sortGenre,
		});

		// Check if discover information exists
		if (!discover) {
			return new HttpError({
